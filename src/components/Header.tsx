...
export function Header({ onShowLogin, onShowSignup }: HeaderProps) {
  const { language, setLanguage, theme, toggleTheme, t, scrollToTop } = useLanguage();
    ...
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        >
          <motion.a 
            href="#" 
            whileHover={{ y: -2, color: theme === "dark" ? "#fff" : "#000" }}
        ...
        >
          ...
          <motion.select 
            ...
            whileTap={{ scale: 0.95 }}
            className="text-sm text-black/70 dark:text-white/70 bg-transparent border-none outline-none cursor-pointer"
          >
          ...
          {user ? (
            <motion.div className="flex items-center gap-3">
              <motion.div 
                className="flex items-center gap-2 px-3 py-2 bg-black/5 dark:bg-white/10 rounded-lg"
